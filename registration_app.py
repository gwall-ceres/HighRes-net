...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def load_image_from_path(self, image_type, filepath):
        ...
        # Update the corresponding pixmap attribute
        #setattr(self, f"{image_type}", stretched_pixmap)
    ...
    def apply_shift_and_update_overlay(self):
        ...
        """
        if self.template_pixmap is None or self.ref_pixmap is None:
            logging.warning("Cannot apply shift: one or both pixmaps are None.")
            return
        ...
        #logging.debug("Converted pixmaps to QImage Format_RGB32 for shifting.")

        # Convert QImage to NumPy arrays
        template_buffer = template_qimage.bits().asstring(template_qimage.byteCount())
        template_array = np.frombuffer(template_buffer, dtype=np.uint8).reshape((template_qimage.height(), template_qimage.width(), 4))[:, :, :3]
        #logging.debug("Converted template QImage to NumPy array.")

        ref_buffer = ref_qimage.bits().asstring(ref_qimage.byteCount())
        ref_array = np.frombuffer(ref_buffer, dtype=np.uint8).reshape((ref_qimage.height(), ref_qimage.width(), 4))[:, :, :3]
        #logging.debug("Converted reference QImage to NumPy array.")
        ...
        bytes_per_line = 3 * width
        shifted_qimage = QtGui.QImage(shifted_template.data, width, height, bytes_per_line, QtGui.QImage.Format_RGB888).copy()
    ...
    def compute_difference_heatmap(self):
        ...
        bytes_per_line = 3 * width
        heatmap_qimage = QtGui.QImage(colored_diff.data, width, height, bytes_per_line, QtGui.QImage.Format_RGB888).copy()
